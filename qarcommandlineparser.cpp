...
#include "qarcommandlineparser.h"
...
class QARCommandLineParserPrivate
{
public:
    inline QARCommandLineParserPrivate()
        : singleDashWordOptionMode(QARCommandLineParser::ParseAsCompactedShortOptions),
    ...
    QARCommandLineParser::SingleDashWordOptionMode singleDashWordOptionMode;
...
QStringList QARCommandLineParserPrivate::aliases(const QString &optionName) const
{
    const NameHash_t::const_iterator it = nameHash.find(optionName);
    if (it == nameHash.end()) {
        qWarning("QARCommandLineParser: option not defined: \"%s\"", qPrintable(optionName));
        return QStringList();
    }
...
/*!
    \since 5.2
    \class QARCommandLineParser
    \inmodule QtCore
    \ingroup tools

    \brief The QARCommandLineParser class provides a means for handling the
    command line options.

    QCoreApplication provides the command-line arguments as a simple list of strings.
    QARCommandLineParser provides the ability to define a set of options, parse the
    ...
    happens before QARCommandLineParser exists, so it can't take it into account. This
    ...
    QCoreApplication::arguments() before QARCommandLineParser defines the \c{profile}
...
*/
QARCommandLineParser::QARCommandLineParser()
    : d(new QARCommandLineParserPrivate)
{
}
...
/*!
    \enum QARCommandLineParser::SingleDashWordOptionMode
...
*/
void QARCommandLineParser::setSingleDashWordOptionMode(QARCommandLineParser::SingleDashWordOptionMode singleDashWordOptionMode)
{
 ...
 */
bool QARCommandLineParser::addOption(const QCommandLineOption &option)
{
...
/*!
    ...
    This option is handled automatically by QARCommandLineParser.
...
*/
QCommandLineOption QARCommandLineParser::addVersionOption()
{
...
/*!
    ...
    Returns the option instance, which can be used to call isSet().
*/
QCommandLineOption QARCommandLineParser::addHelpOption()
{
...
*/
void QARCommandLineParser::setApplicationDescription(const QString &description)
{
...
*/
QString QARCommandLineParser::applicationDescription() const
{
...
*/
void QARCommandLineParser::addPositionalArgument(const QString &name, const QString &description, const QString &syntax)
{
    QARCommandLineParserPrivate::PositionalArgumentDefinition arg;
    arg.name = name;
...
*/
void QARCommandLineParser::clearPositionalArguments()
{
...
/*!
    ...
    \sa process()
*/
bool QARCommandLineParser::parse(const QStringList &arguments)
{
...
*/
QString QARCommandLineParser::errorText() const
{
 ...
 */
void QARCommandLineParser::process(const QStringList &arguments)
{
 ...
 */
void QARCommandLineParser::process(const QCoreApplication &app)
{
...
void QARCommandLineParserPrivate::checkParsed(const char *method)
{
    if (needsParsing)
        qWarning("QARCommandLineParser: call process() or parse() before %s", method);
}
 ...
 */
bool QARCommandLineParserPrivate::registerFoundOption(const QString &optionName)
{
...
/*!
    ...
    Returns \c true on success.
 */
bool QARCommandLineParserPrivate::parseOptionValue(const QString &optionName, const QString &argument,
        ...
        if (withValue) {
            if (assignPos == -1) {
                ...
                if (*argumentIterator == argsEnd) {
                    errorText = QARCommandLineParser::tr("Missing value after '%1'.").arg(argument);
                    return false;
                }
            ...
            if (assignPos != -1) {
                errorText = QARCommandLineParser::tr("Unexpected value after '%1'.").arg(argument.left(assignPos));
                return false;
            }
        }
    }
 ...
 */
bool QARCommandLineParserPrivate::parse(const QStringList &args)
{
    needsParsing = false;
    bool error = false;
    ...
    if (args.isEmpty()) {
        qWarning("QARCommandLineParser: argument list cannot be empty, it should contain at least the executable name");
        return false;
    }
    ...
    for (; argumentIterator != args.end() ; ++argumentIterator) {
        ...
        if (doubleDashFound) {
            ...
            case QARCommandLineParser::ParseAsCompactedShortOptions:
            {
                QString optionName;
            ...
            case QARCommandLineParser::ParseAsLongOptions:
            {
...
QStringList QARCommandLineParser::positionalArguments() const
{
...
QStringList QARCommandLineParser::optionNames() const
{
...
QStringList QARCommandLineParser::unknownOptionNames() const
{
...
/*!
    ...
    \sa helpText()
*/
Q_NORETURN void QARCommandLineParser::showHelp(int exitCode)
{
...
/*!
    ...
    \sa showHelp()
*/
QString QARCommandLineParser::helpText() const
{
    return d->helpText();
}
...
QString QARCommandLineParserPrivate::helpText() const
{
    const QLatin1Char nl('\n');
    QString text;
    ...
    if (!commandLineOptionList.isEmpty()) {
        usage += QLatin1Char(' ');
        usage += QARCommandLineParser::tr("[options]");
    }
    foreach (const PositionalArgumentDefinition &arg, positionalArgumentDefinitions) {
        usage += QLatin1Char(' ');
        usage += arg.syntax;
    }
    text += QARCommandLineParser::tr("Usage: %1").arg(usage) + nl;
    if (!description.isEmpty())
       text += description + nl;
    text += nl;
    if (!commandLineOptionList.isEmpty())
        text += QARCommandLineParser::tr("Options:") + nl;
    ...
    if (!positionalArgumentDefinitions.isEmpty()) {
        if (!commandLineOptionList.isEmpty())
            text += nl;
        text += QARCommandLineParser::tr("Arguments:") + nl;
        foreach (const PositionalArgumentDefinition &arg, positionalArgumentDefinitions) {
            text += wrapText(arg.name, longestOptionNameString, arg.description);
        }
    }

    text.append( moreHelp );

    return text;
}


void QARCommandLineParser::appendToHelp( const QString& val ) {
  if( d->moreHelp.isEmpty() )
    d->moreHelp.append( "\n\n" );

  d->moreHelp.append( val );
  d->moreHelp.append( "\n" );
}
