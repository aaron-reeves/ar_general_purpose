...
static double NumFromRk(XLSBYTE* rk);
...
extern void xls_addCell(xlsWorkSheet* pWS,BOF* bof,XLSBYTE* buf);
...
extern void xls_mergedCells(xlsWorkSheet* pWS,BOF* bof,XLSBYTE* buf);
...
void xls_addSST(xlsWorkBook* pWB,SST* sst,XLSDWORD size)
{
    verbose("xls_addSST");
...
void xls_addCell(xlsWorkSheet* pWS,BOF* bof,XLSBYTE* buf)
{
    ...
    case 0x06:	//FORMULA
        ...
        break;
    case 0x0BD:	//MULRK
        for (i=0;i<=*(XLSWORD *)(buf+(bof->size-2))-((COL*)buf)->col;i++)
        {
            cell=&row->cells.cell[((COL*)buf)->col-row->fcell+i];
            //				col=row->cols[i];
            cell->id=bof->id;
            cell->xf=*((XLSWORD *)(buf+(4+i*6)));
            cell->d=NumFromRk((XLSBYTE *)(buf+(4+i*6+2)));
            cell->str=xls_getfcell(pWS->workbook,cell);
        }
        break;
    case 0x0BE:	//MULBLANK
        for (i=0;i<=*(XLSWORD *)(buf+(bof->size-2))-((COL*)buf)->col;i++)
        {
            ...
            //				col=row->cols[i];
            cell->id=bof->id;
            cell->xf=*((XLSWORD *)(buf+(4+i*2)));
            cell->str=xls_getfcell(pWS->workbook,cell);
        }
        break;
    case 0xFD:	//LABELSST
        cell->l=*(XLSWORD *)&((LABELSST*)buf)->value;
        cell->str=xls_getfcell(pWS->workbook,cell);
        break;
    case 0x27E:	//RK
...
char *xls_addFont(xlsWorkBook* pWB,FONT* font)
{
    ...
    tmp->name=get_string((XLSBYTE*)&font->name,0,0,pWB->charset);	// TODO: unicode relevant here?
...
void xls_addFormat(xlsWorkBook* pWB,FORMAT* format)
{
    ...
    tmp->value=get_string((XLSBYTE*)format + 2,0,1,pWB->charset);	// TODO: unicode relevant here?

    //printf( "xls_addFormat Index: %i, value: %s\n", tmp->index, tmp->value );
...
void xls_mergedCells(xlsWorkSheet* pWS,BOF* bof,XLSBYTE* buf)
{
    (void)bof; // Eliminates "-Wunused-parameter" warning.
    int count=*((XLSWORD*)buf);
    int i,c,r;
...
void xls_parseWorkBook(xlsWorkBook* pWB)
{
    ...
    do {
        ...
        case 0x809:		// BIFF5-8
            ...
            break;
        case 0x22: // 1904 date system
            pWB->is1904datesystem = *(XLSWORD*)buf;
            //printf( "Is 1904: %i\n", pWB->is1904datesystem );
            break;
        case 0x042:		// CODEPAGE
            pWB->codepage=*(XLSWORD*)buf;
            break;
        case 0x3c:		// CONTINUE
            if(once) {
              if (bof2.id==0xfc)
        ...
        case 0x85:		// BOUNDSHEET
            {
              ...
              // AR 19 Feb 2017: Block above changed to the following, to prevent a compiler warning.
              BOUNDSHEET *bs = (BOUNDSHEET *)buf;
              // different for BIFF5 and BIFF8
              xls_addSheet(pWB,bs);
            }
