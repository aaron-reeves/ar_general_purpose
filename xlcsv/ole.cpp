...
#include <ar_general_purpose/xlcsv/xlstool.h>

#pragma pack(1)

extern int xls_debug;

const XLSDWORD MSATSECT =		0xFFFFFFFC;	// -4
const XLSDWORD FATSECT =		0xFFFFFFFD;	// -3
const XLSDWORD ENDOFCHAIN =	0xFFFFFFFE;	// -2
const XLSDWORD FREESECT =		0xFFFFFFFF;	// -1

static int sector_pos(OLE2* ole2, int sid);
static int sector_read(OLE2* ole2, XLSBYTE *buffer, int sid);
...
int ole2_read(void* buf,long size,long count,OLE2Stream* olest)
{
	...
	while ((!olest->eof) && (didReadCount!=totalReadCount))
	{
		...
		if (needToReadCount < remainingBytes)	// does the current sector contain all the data I need?
		{
			...
			memcpy((XLSBYTE*)buf + didReadCount, olest->buf + olest->pos, needToReadCount);
			olest->pos		+= needToReadCount;
			didReadCount	+= needToReadCount;
		} else {
			// printf("  had %d bytes of memory, copy=%d\n", remainingBytes, remainingBytes);
			memcpy((XLSBYTE*)buf + didReadCount, olest->buf + olest->pos, remainingBytes);
...
OLE2Stream* ole2_sopen(OLE2* ole,XLSDWORD start, int size)
{
  ...
  olest->buf=(XLSBYTE*)malloc(olest->bufsize);
	ole2_bufread(olest);
...
// Move in stream
void ole2_seek(OLE2Stream* olest,XLSDWORD ofs)
{
	if(olest->sfat) {
...
// Open physical file
OLE2* ole2_open(const char *file, const char *charset)
{
    //BYTE buf[1024];
    OLE2Header* oleh;
    OLE2* ole;
  ...
  oleh = NULL;
    ...
    do
    {
        ...
        if (pss->type == PS_USER_ROOT || pss->type == PS_USER_STREAM) // (name!=NULL) // 
        {
            ...
            {
                ole->files.file=(st_olefiles_data*)malloc(sizeof(struct st_olefiles_data));
            } else {
                ole->files.file=(st_olefiles_data*)realloc(ole->files.file,(ole->files.count+1)*sizeof(struct st_olefiles_data));
            }
			...
			if(pss->type == PS_USER_ROOT) {
				XLSDWORD sector, k, blocks;
				XLSBYTE *wptr;
				
				blocks = (pss->size + (ole->lsector - 1)) / ole->lsector;	// count partial
				ole->SSAT = (XLSBYTE *)malloc(blocks*ole->lsector);
				// printf("blocks %d\n", blocks);

				assert(ole->SSecID);
				
				sector = pss->sstart;
				wptr=(XLSBYTE*)ole->SSAT;
				for(k=0; k<blocks; ++k) {
...
static int read_MSAT(OLE2* ole2, OLE2Header* oleh)
{
    ...
    ole2->SecID=(XLSDWORD*)malloc(ole2->cfat*ole2->lsector);
    ...
    // read first 109 sectors of MSAT from header
    {
        ...
        {
            sector_read(ole2, (XLSBYTE*)(ole2->SecID)+sectorNum*ole2->lsector, oleh->MSAT[sectorNum]);
        }
    }
    ...
    // Add additionnal sectors of the MSAT
    {
        ...
        XLSBYTE *sector = (XLSBYTE*)malloc(ole2->lsector);
           ...
           for (posInSector = 0; posInSector < (ole2->lsector-4)/4; posInSector++)
             {
              ...
              if (s != FREESECT)
                {
                 sector_read(ole2, (XLSBYTE*)(ole2->SecID)+sectorNum*ole2->lsector, s);
                 sectorNum++;
                }
             }
	...
	if(ole2->sfatstart != ENDOFCHAIN) {
		...
		ole2->SSecID = (XLSDWORD *)malloc(ole2->csfat*ole2->lsector);
		sector = ole2->sfatstart;
		wptr=(XLSBYTE*)ole2->SSecID;
