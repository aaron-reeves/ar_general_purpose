...
void QCodecs::base64Encode( const QByteArray & in, QByteArray & out, bool insertLFs ) {
  // clear out the output buffer
  out.resize( 0 );
  if( in.isEmpty(  ) )
    return;
  ...
  if( len > 1 ) {
    while( sidx < len - 2 ) {
      ...
      out.data(  )[didx++] = Base64EncMap[( data[sidx + 1] >> 4 ) & 017 | ( data[sidx] << 4 ) & 077];
      out.data(  )[didx++] = Base64EncMap[( data[sidx + 2] >> 6 ) & 003 | ( data[sidx + 1] << 2 ) & 077];
      out.data(  )[didx++] = Base64EncMap[data[sidx + 2] & 077];
      sidx += 3;
    }
  }
...
void QCodecs::uuencode( const QByteArray & in, QByteArray & out ) {
  ...
  while( sidx + line_len < len ) {
    ...
    for( unsigned int end = sidx + line_len; sidx < end; sidx += 3 ) {
      ...
      out.data(  )[didx++] = UUEncMap[( data[sidx + 1] >> 4 ) & 017 | ( data[sidx] << 4 ) & 077];
      out.data(  )[didx++] = UUEncMap[( data[sidx + 2] >> 6 ) & 003 | ( data[sidx + 1] << 2 ) & 077];
      out.data(  )[didx++] = UUEncMap[data[sidx + 2] & 077];
    }
...
QCString QMD5::hexDigest(  ) {
  ...
  sprintf( s.data(  ), "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
    ...
    m_digest[6], m_digest[7], m_digest[8], m_digest[9], m_digest[10], m_digest[11], m_digest[12], m_digest[13], m_digest[14], m_digest[15] );
...
void QMD5::FF( Q_UINT32 & a, Q_UINT32 b, Q_UINT32 c, Q_UINT32 d, Q_UINT32 x, Q_UINT32 s, Q_UINT32 ac ) {
  a += F( b, c, d ) + x + ac;
  a = rotate_left( a, s ) + b;
}


void QMD5::GG( Q_UINT32 & a, Q_UINT32 b, Q_UINT32 c, Q_UINT32 d, Q_UINT32 x, Q_UINT32 s, Q_UINT32 ac ) {
  a += G( b, c, d ) + x + ac;
  a = rotate_left( a, s ) + b;
}


void QMD5::HH( Q_UINT32 & a, Q_UINT32 b, Q_UINT32 c, Q_UINT32 d, Q_UINT32 x, Q_UINT32 s, Q_UINT32 ac ) {
  a += H( b, c, d ) + x + ac;
  a = rotate_left( a, s ) + b;
}


void QMD5::II( Q_UINT32 & a, Q_UINT32 b, Q_UINT32 c, Q_UINT32 d, Q_UINT32 x, Q_UINT32 s, Q_UINT32 ac ) {
  a += I( b, c, d ) + x + ac;
  a = rotate_left( a, s ) + b;
}
...
void QMD5::decode( Q_UINT32 * output, const unsigned char *in, Q_UINT32 len ) {
  ...
  for( i = 0, j = 0; j < len; i++, j += 4 )
    output[i] = static_cast < Q_UINT32 > ( in[j] ) |
      ( static_cast < Q_UINT32 > ( in[j + 1] ) << 8 ) | ( static_cast < Q_UINT32 > ( in[j + 2] ) << 16 ) | ( static_cast < Q_UINT32 > ( in[j + 3] ) << 24 );
#endif
}
